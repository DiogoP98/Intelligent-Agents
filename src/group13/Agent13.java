...
//            if (time > 0.3) concessionRate = 0.1;
//            if (time > 0.9) concessionRate = 0.20;
//            if (time > 0.95) concessionRate = 0.27;

            double utilityThreshold = getUtilityThreshold();

            if(time > 0.9 && lastReceivedOffer != null){
                try {
                    if(bidHistoryAgent.size() > n){
                        lastNAgentBids = bidHistoryAgent.subList(bidHistoryAgent.size() - n, bidHistoryAgent.size() - 2);
                        lastNOpponentBids = bidHistoryOpponent.subList(bidHistoryOpponent.size() - n, bidHistoryOpponent.size() - 2);
                    }
                    else{
                ...
                boolean suddenAgentIncrease = detectSuddenChange(lastNAgentBids, this.utilitySpace.getUtility(lastReceivedOffer), 3);
                boolean suddenOpponentIncrease = detectSuddenChange(lastNOpponentBids, this.opponent.getValue(lastReceivedOffer), 3);
            ...
            Set<Bid> bidSet = this.generateBids(utilityThreshold, 5000, 30000);
